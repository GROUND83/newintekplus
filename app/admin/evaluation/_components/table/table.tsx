...
import { getLiveSurvey } from "./actions";
import {
  Select,
  SelectContent,
  SelectItem,
...
export default function DataTable() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      let reponse = await getLiveSurvey(fetchDataOptions);
      console.log("response", reponse.pageCount);
      if (reponse.rows) {
        let liveSurvey = JSON.parse(reponse.rows);
        console.log("liveSurvey", liveSurvey);
        return { rows: liveSurvey, pageCount: reponse.pageCount };
      }
    },
  });
